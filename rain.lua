...
function Rain:new (o, width, height, deltaX, deltaY, rainAmount)
  ...
  for i=1,self.rainAmount do
    ...
    self.rain[i].speed = love.math.random(500, 1000)
    ...
    self.rain[i].color = love.math.random(1,10)/10
    self.rain[i].limit = self.height - self.height/(love.math.random(4, 20))

    self.drops[i].radius = 1
  end
...
function Rain:update (dt)
  for i=1,self.rainAmount do
    ...
    if self.drops[i].thing then
      self.drops[i].radius = self.drops[i].radius + 0.5
      if self.drops[i].radius >= 10 then
...
function Rain:draw ()
  for i=1,self.rainAmount do
    -- Draw the rain drops
    love.graphics.setColor(self.rain[i].color, self.rain[i].color, self.rain[i].color)
